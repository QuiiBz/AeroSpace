...
@MainActor private func changeTilingLayoutOrConvertFromFloating(_ io: CmdIo, targetLayout: Layout?, targetOrientation: Orientation?, window: Window) async throws -> BinaryExitCode {
    guard let parent = window.parent else { return .fail }
    switch parent.cases {
        case .tilingContainer(let parent):
            let targetOrientation = targetOrientation ?? parent.orientation
